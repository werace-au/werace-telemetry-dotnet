...
public class MetadataTests
{
    ...
    public void MetadataWithMultipleEntriesPreservesAlignment()
    {
        ...
        // Verify session header starts on 8-byte boundary
    ...
    [Fact]
    public void MetadataWithMaxSizeValues()
    {
        using var stream = new MemoryStream();

        // Create large strings that will need significant padding
        var largeKey1 = new string('k', 1000);    // 1000 character key
        ...
        // Verify session starts on 8-byte boundary
        var sessionHeaderStart = stream.Position - Magic.MAGIC_SIZE;
    ...
    {
        using var stream = new MemoryStream();
        var writer = new Writer<TestSession, TestFrame>(stream, 60);
        ...
        var sessionHeaderStart = stream.Position - Magic.MAGIC_SIZE; // Subtract magic number size
        Assert.True(sessionHeaderStart % 8 == 0, "Session header should be 8-byte aligned");
